...
<?php
	$posttag = get_theme_mod('ecomodernist_front_section_four_tag');
	$tag_link = get_tag_link( $posttag );

	$postcat = get_theme_mod('ecomodernist_front_section_four_cat');
	$category_link = get_category_link($postcat);

	$ecomodernist_section_four_query = new WP_Query( array(
		'posts_per_page'			=> 1,
		'post_status'					=> 'publish',
		'tag_id' 							=> $posttag,
		'cat' 								=> $postcat,
...
<section id="front-section-four" class="front-section cf">
	...
	<?php if($ecomodernist_section_four_query->have_posts()) : ?>
		<?php while($ecomodernist_section_four_query->have_posts()) : $ecomodernist_section_four_query->the_post() ?>

		<article id="post-<?php the_ID(); ?>" <?php post_class('cf'); ?>>

			<?php if ( 'neo' == get_theme_mod('ecomodernist_main_design') && '' != get_the_post_thumbnail() && ! post_password_required() ) : ?>
				<div class="entry-thumbnail fadein"><a href="<?php the_permalink(); ?>"><span class="thumb-wrap"><?php the_post_thumbnail('ecomodernist-bigthumb'); ?></span></a></div><!-- end .entry-thumbnail -->
			<?php elseif ( '' != get_the_post_thumbnail() && ! post_password_required() ) : ?>
				<div class="entry-thumbnail fadein"><a href="<?php the_permalink(); ?>"><span class="thumb-wrap"><?php the_post_thumbnail('ecomodernist-featured'); ?></span></a></div><!-- end .entry-thumbnail -->
			<?php endif; ?>
